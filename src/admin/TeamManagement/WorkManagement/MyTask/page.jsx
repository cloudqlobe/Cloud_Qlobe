...
export default function ManagerTaskAssignment() {
  ...
  const fetchTasks = async () => {
    try {
      const res = await axiosInstance.get("api/admin/member-tasks");
      setTasks(res.data);
  ...
  const fetchMembers = async () => {
    try {
      const res = await axiosInstance.get("api/admin/sale-members");
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      if (editingTask) {
        ...
        await axiosInstance.put(`api/admin/member-tasks/${editingTask.id}`, updatedTask);
        ...
        // ✅ Create Task
        const res = await axiosInstance.post('api/admin/createmembertask', taskData);
  ...
  const handleDelete = async (taskId) => {
    ...
    try {
      await axiosInstance.delete(`api/admin/member-tasks/${taskId}`);
  ...
  const handleCheckTask = async (taskId) => {
    try {
      const res = await axiosInstance.put(`api/admin/member-tasks/${taskId}/check`);
  ...
  const handleConfirmTaskCompletion = async (taskId) => {
    try {
      await axiosInstance.delete(`api/admin/member-tasks/${taskId}/confirm`);
              ...
              <div key={task.id} className="bg-white rounded-2xl shadow-xl p-6 hover:shadow-2xl transition-all">
                ...
                <div>
                  ...
                  <div className="grid grid-cols-1 sm:grid-cols-3 gap-3">
                    {task.filteredMembers.slice(0, 4).map((member, i) => (
                      ...
                      >
                        ...
                        <div className="text-right">
                          ...
                          {/* 🧮 Show Extra Work if any */}
                          {extraWork[member.id] > 0 && (
                            <p className="text-xs text-purple-600 mt-1 font-semibold">
                              Extra Task: {extraWork[member.id]}
                            </p>
                          )}
                        </div>
                      </div>
                    ...
                    {task.assignedToAll.every(m => m.taskstatus === "complete") && (
